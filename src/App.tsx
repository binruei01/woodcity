...
7. 校園碳匯科學研究（民族國小 43 屆科展「校園碳險隊」成果）：
   - 研究目的：透過普查校園樹木並自製物聯網（IoT）感測儀器，分析樹木的固碳能力與環境影響因素。
   - 研究對象：校園內 28 種樹種（包含原生種 8 種、外來種 17 種、特有種 3 種），共計 120 棵樹木。
   - 核心數據：校園樹木總固碳量約為 107,603.01 kg CO2e，相當於節省了約 21.7 萬度電。
   - 固碳高手排行：台灣相思樹 (50-80kg/年) 第一，其次是樟樹 (40-60kg/年)、楓香 (30-50kg/年) 與小葉欖仁 (30-50kg/年)。
   - 科學發現：透過 MH-Z19B 二氧化碳感測器與 ThingSpeak 平台監測發現，中午 11-12 點因光合作用最旺盛，校園 CO2 濃度最低；早晨 7-8 點濃度則最高（約 430ppm）。
   - 樹種比較：研究發現楓香在春季因葉片茂密，其吸收 CO2 的效能表現優於樟樹。
   - 工具與方法：使用「校園樹木資訊平台」進行數據錄入，並利用自製仰角觀測器與三角函數計算樹高，在 1.3 公尺處測量胸高直徑 (DBH)。
   - 這是將在地木文化轉化為科學實踐，推動 AI 智慧綠色校園的具體案例。

如果使用者的問題與嘉義木都、東市場、阿里山林務、木造建築、校園樹木碳匯研究、嘉義歷史等主題完全無關，請務必回答：『這個問題超過我的知識範圍。你可以先試著問問：木都、東市場、檜木、校園碳匯、老屋、科展研究等話題喔！』。
    ...
    <div className="min-h-screen bg-[#fffaf0] font-sans text-gray-800 selection:bg-amber-200">
      ...
      <nav className="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-amber-100 px-4 py-3">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 font-bold text-[#5d4037]">
            ...
            <span className="hidden sm:inline">探索嘉義木都文化與在地導覽AI查詢平台</span>
            <span className="sm:hidden">木都導覽與 AI 查詢</span>
          </div>
      ...
      <header className="relative bg-[#5d4037] text-white overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto py-16 px-6 text-center relative z-10">
          ...
          >
            探索嘉義木都文化與在地導覽<br className="hidden md:block" /> AI 查詢平台
          </motion.h1>
          <motion.p 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
